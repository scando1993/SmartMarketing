...
from AudioCortex import audio_cortex
import re
import os
import urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, json, requests
...
def playMusic(query):
    ...
    query = query[0]
    print(query)
    url = "https://www.googleapis.com/youtube/v3/search?part=snippet&key=" + keyring.get_password('google',
                                                                                                  'api_secret') + "&q=" + urllib.parse.quote_plus(
        query) + "&type=video"
...
def wikipediaLookUp(a_string, num_sentences):
    ...
    print(phrase)
    pattern = re.compile(
        "\\b(lot|lots|a|an|who|can|you|what|is|info|somethings|whats|have|i|something|to|know|like|Id|information|about|tell|me)\\W",
        re.I)
    phrase_noise_removed = [pattern.sub("", phrase)]
